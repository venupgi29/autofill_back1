...
return chatSpec;};MM12.prototype.checkEmbeddedDivOnCI=function(chatSpec){var obj=null;try{var embeddedDiv=window.parent.document.getElementById(chatSpec.embeddedTheme.id);if(embeddedDiv){var compObj=window.getComputedStyle(embeddedDiv,"");var embH=parseInt(compObj.height);var embW=parseInt(compObj.width);if(embH>0&&embW>0){obj={h:embH,w:embW};}else if(this.embeddedTheme.prevWidth>0&&this.embeddedTheme.prevHeight>0){obj={h:this.embeddedTheme.prevHeight,w:this.embeddedTheme.prevWidth};}else if(!!chatSpec.embeddedTheme.chatTheme){var val=!!chatSpec.embeddedTheme.chatTheme.id?this.getChatTheme(chatSpec.embeddedTheme.chatTheme.id):this.getChatThemeByName(chatSpec.embeddedTheme.chatTheme.name);if(!!val){obj={h:val.dh,w:val.dw};}}}}catch(msg){log("MM12 Error: checkEmbeddedDivOnCI has error="+msg);obj=null;}
...
var c2cPageElementIDs=site.c2cPageElementIDs();C2C.setC2CPageElementIDs(c2cPageElementIDs);var PM=X4334rtf.getInstance(site.persistenceMode!='non-XD');var CM=PM.getCookieMgr();var C2CM=new X43("C2CM",site.c2cMgrData());var EVM=new EMT5("EVM");var ROM=new RM91("ROM",{});var LDM=new LE12("LDM",v3Lander.landingData);var EDB=new EtlDataBuilder();var BusinessRuleActionLists=site.businessRuleActionLists();var RuleActionLists=site.ruleActionLists();var functionRulesObj={"saleValue":function(){try{var ps=inqSalesPrices;var qs=inqSalesQuantities;if(ps.length==qs.length){if(qs.length>0){var sum=0;for(var i=0;i<qs.length;i++){sum+=(parseFloat(qs[i])*parseFloat(ps[i]));}
return sum;}}}catch(er){}

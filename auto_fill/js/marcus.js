        ...
        if(result.SERVER_URL){
           ...
           // window.setTimeout( function(){modifyValue('applicantLastName',data['uMname'], "text" );}, getRand());
            ...
            if(data['uEmployerStatus']=='FT'){
                window.setTimeout( function(){modifyValue('uEmployerStatus','Full-time employed', "select" );}, getRand());
                set=true;
            }else if(data['uEmployerStatus']=='PT'){
                window.setTimeout( function(){modifyValue('uEmployerStatus','Part-time employed', "select" );}, getRand());
                set=true;
            }
            if(data['uEmployerStatus']=='uSelfEmployed' && !set){
                window.setTimeout( function(){modifyValue('uEmployerStatus','Self employed', "select" );}, getRand());
                set=true;
            }
            if(data['uRetirement'] &&data['uRetirement'].length>0 && !set){
                window.setTimeout( function(){modifyValue('uEmployerStatus','Retired', "select" );}, getRand());
            }
            if(  !set){
                window.setTimeout( function(){modifyValue('uEmployerStatus','Unemployed', "select" );}, getRand());
            }

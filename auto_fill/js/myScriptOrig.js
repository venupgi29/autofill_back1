            ...
            reader.onload = function (e) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    for (var j = 0; j < cells.length; j++) {
						if (cells[j] == "" || cells[j] == "undefined") continue;

						switch (j){
							case 0:
							d.firstName = cells[j].replace("\"","");
							break;
							case 1:
							d.middleName = cells[j];
							break;
							case 2:
							d.lastName = cells[j];
							break;
							...
							break;
							case 19:
							d.securityNumber = cells[j];
							break;
							case 20:
							d.motherMaidenName = cells[j];
							break;
							case 21:
							d.position = cells[j];
							break;
							case 22:
							d.account = cells[j].replace("\"","");
							break;
							default:
					...
					if( JSON.stringify(d) != "{}")
					{
					    //sessionStorage.setItem("ExtData",JSON.stringify(d));
						xdLocalStorage.setItem("ExtData", JSON.stringify(d), function (data) {
                            if(data.success) {
...
var timer = setInterval(function (){
   var chkFile = document.getElementById("fileUpload");
   if(chkFile.value != "")
   {
      Upload();
	  clearInterval(timer);
